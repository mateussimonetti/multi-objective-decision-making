...
def neighborhoodChange(x_, x_line_, k, f, w):
    y_ = f(x_,w)
    y_line_ = f(x_line_,w)
...
def bestImprovement(dados, f, w):
    ...
    f_1 = float(f(dados,w))
    f_2 = float(f(new_dados_1, w))
    f_3 = float(f(new_dados_2, w))
...
def BVNS(dados, f, w, k_max, max_int = 100, plot = False, n_plot = 10000, dir_save = './'):
    nfe = 0
    ...
    while(nfe<max_int):
        ...
        while(k<=k_max):
            ...
            dados_line_line = bestImprovement(copy.copy(dados_line), f, w)
            #update x
            dados, k = neighborhoodChange(copy.copy(dados), copy.copy(dados_line_line), k, f, w)
            #save 
            y = f(dados, w)
            #update log
...
def problemaPw(dados, n_sol, k_max, max_VNS_it, plot = True, dir_save = './'):
    ...
    for i in range(0,n_sol):
        ...
        dados_res, log, time_total = BVNS(copy.copy(dados_init), pw, w, k_max, max_VNS_it, plot = False)
