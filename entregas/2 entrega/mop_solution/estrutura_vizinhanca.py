import numpy as np
from restricoes import calcular_distancias_cliente_PA
...
def k1(dados):
    ...
    PAs_ativos = np.where(dados['uso_PAs'] == 1)[0]
    clientes_ativos_por_PA = dados['cliente_por_PA'][:, PAs_ativos].sum(axis=0)
    PA_menos_clientes = PAs_ativos[np.argmin(clientes_ativos_por_PA)]

    # Desativa o PA com menos clientes
    dados['uso_PAs'][PA_menos_clientes] = 0
    PAs_ativos = np.where(dados['uso_PAs'] == 1)[0]
    ...
    for cliente in clientes_desalocados:
        ...
        distancias = np.linalg.norm(dados['coord_clientes'][cliente] - dados['possiveis_coord_PA'][PAs_ativos], axis=1)
    ...
    dados['dist_cliente_PA'] = calcular_distancias_cliente_PA(dados['possiveis_coord_PA'][PAs_ativos], dados['cliente_por_PA'][:, PAs_ativos])
...
def k2(dados):
    ...
    PAs_ativos = np.where(dados['uso_PAs'] == 1)[0]
    posicoes_PAs_ativos = dados['possiveis_coord_PA'][PAs_ativos]
    PAs_na_regiao = np.where(
        ((posicoes_PAs_ativos[:, 0] >= x_inicio) & (posicoes_PAs_ativos[:, 0] < x_fim)) &
        ((posicoes_PAs_ativos[:, 1] >= y_inicio) & (posicoes_PAs_ativos[:, 1] < y_fim))
    )[0]

    # Verifica se há PAs ativos na região
    if len(PAs_na_regiao) > 0:
        # Encontra o PA ativo com menos clientes na região
        clientes_por_PA_na_regiao = dados['cliente_por_PA'][:, PAs_ativos[PAs_na_regiao]]
        clientes_ativos_por_PA = clientes_por_PA_na_regiao.sum(axis=0)
        PA_menos_clientes_indice = np.argmin(clientes_ativos_por_PA)
        PA_menos_clientes = PAs_ativos[PAs_na_regiao[PA_menos_clientes_indice]]

        # Desabilita o PA com menos clientes na região
        dados['uso_PAs'][PA_menos_clientes] = 0
        PAs_ativos = np.where(dados['uso_PAs'] == 1)[0]

        # Obtém os clientes do PA desabilitado
        clientes_desalocados = np.where(dados['cliente_por_PA'][:, PA_menos_clientes] == 1)[0]

        # Tenta realocar os clientes desalocados
        for cliente in clientes_desalocados:
            # Remove a conexão do cliente com o PA desabilitado
            dados['cliente_por_PA'][cliente, PA_menos_clientes] = 0

            # Encontra todos os PAs ativos dentro do limite de alcance do cliente
            distancias_cliente = np.linalg.norm(
                dados['coord_clientes'][cliente] - dados['possiveis_coord_PA'][PAs_ativos], axis=1
            )
            PAs_ativos_alcance = PAs_ativos[np.where(distancias_cliente <= dados['limite_sinal_PA'])[0]]

            # Tenta conectar ao PA ativo com capacidade disponível
            for PA in PAs_ativos_alcance:
                consumo_atual_pa = np.sum(dados['cliente_por_PA'][:, PA] * dados['cons_clientes'])
                if consumo_atual_pa + dados['cons_clientes'][cliente] <= dados['capacidade_PA']:
                    dados['cliente_por_PA'][cliente, PA] = 1
                    break
        ...
        dados['dist_cliente_PA'] = calcular_distancias_cliente_PA(
            dados['possiveis_coord_PA'][PAs_ativos], dados['cliente_por_PA'][:, PAs_ativos]
        )
...
def k3(dados):
    ...
    for cliente in clientes_na_regiao:
        ...
        PAs_ordenados_por_distancia = PAs_ativos[np.argsort(distancias)]
...
def k4(dados):
    ...
    PAs_inativos = np.where(dados['uso_PAs'] == 0)[0]

    # Verifica se existem PAs inativos para habilitar
    if len(PAs_inativos) > 0:
        # Escolhe aleatoriamente um PA inativo para habilitar
        PA_a_habilitar = np.random.choice(PAs_inativos)

        # Define uma posição aleatória para o PA dentro do grid
        nova_posicao = (
            np.random.randint(0, dados['sizex'][1]),
            np.random.randint(0, dados['sizey'][1])
        )

        # Ajusta o tamanho dos arrays para o máximo permitido se necessário
        if PA_a_habilitar >= len(dados['possiveis_coord_PA']):
            tamanho_atual = len(dados['possiveis_coord_PA'])
            tamanho_maximo = dados['n_max_PAs']
            
            # Extende os arrays para o tamanho máximo
            dados['possiveis_coord_PA'] = np.pad(
                dados['possiveis_coord_PA'], 
                ((0, tamanho_maximo - tamanho_atual), (0, 0)), 
                'constant', 
                constant_values=((0, 0), (0, 0))
            )
            dados['uso_PAs'] = np.pad(
                dados['uso_PAs'], 
                (0, tamanho_maximo - tamanho_atual), 
                'constant'
            )

            # Define a posição e o estado do novo PA
            dados['possiveis_coord_PA'][PA_a_habilitar] = nova_posicao
            dados['uso_PAs'][PA_a_habilitar] = 1
        else:
            # Define a posição e o estado do novo PA
            dados['possiveis_coord_PA'][PA_a_habilitar] = nova_posicao
            dados['uso_PAs'][PA_a_habilitar] = 1

        # Recalcula a matriz de distâncias (considerando o novo PA ativo)
        PAs_ativos = np.where(dados['uso_PAs'] == 1)[0]
        dados['dist_cliente_PA'] = calcular_distancias_cliente_PA(
            dados['possiveis_coord_PA'][PAs_ativos],
            dados['cliente_por_PA'][:, PAs_ativos]
        )

        # Redistribui os clientes, priorizando a redução de distância
        for cliente in range(len(dados['coord_clientes'])):
            # Ordena os PAs por distância
            distancias = dados['dist_cliente_PA'][cliente]
            PAs_ordenados_por_distancia = PAs_ativos[np.argsort(distancias)]
